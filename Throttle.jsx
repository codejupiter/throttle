...
import { Search, ArrowRight, Phone, MessageCircle, Zap, Filter, MapPin, Check, Shield, Clock, ChevronRight } from "lucide-react";

// Throttle — CMS-driven powersports dealership site
// Demo dealer: "Saguaro Powersports" (Tucson, AZ)
// Content layer is structured exactly like a Sanity/Contentful feed would deliver it.
// In production: replace INVENTORY const with `await client.fetch(query)`.

const CMS_INVENTORY = [
  { id: "yzf-r1-2024", category: "sportbike", make: "Yamaha", model: "YZF-R1", year: 2024, price: 18399, mileage: 0, status: "in-stock", color: "Team Yamaha Blue", featured: true, image: "🏍️", specs: { engine: "998cc inline-4", power: "200 hp", weight: "443 lbs" }, tagline: "Track-bred superbike, street-legal" },
  { id: "rzr-xp-2024", category: "utv", make: "Polaris", model: "RZR XP 1000", year: 2024, price: 24995, mileage: 0, status: "in-stock", color: "Indy Red", featured: true, image: "🚙", specs: { engine: "999cc twin", power: "110 hp", seating: "2-seat" }, tagline: "Desert-ready performance UTV" },
  { id: "gti-130-2024", category: "watercraft", make: "Sea-Doo", model: "GTI 130", year: 2024, price: 11499, mileage: 0, status: "in-stock", color: "Coral Blast", featured: false, image: "🌊", specs: { engine: "1630 ACE", power: "130 hp", capacity: "3 riders" }, tagline: "The bestselling rec watercraft" },
  { id: "outlander-850-2023", category: "atv", make: "Can-Am", model: "Outlander 850", year: 2023, price: 9899, mileage: 12, status: "in-stock", color: "Tundra Green", featured: false, image: "🏁", specs: { engine: "854cc twin", power: "78 hp", towing: "1,650 lbs" }, tagline: "All-terrain workhorse" },
  { id: "ninja-zx10r-2024", category: "sportbike", make: "Kawasaki", model: "Ninja ZX-10R", year: 2024, price: 17399, mileage: 0, status: "incoming", color: "Lime Green", featured: false, image: "🏍️", specs: { engine: "998cc inline-4", power: "200 hp", weight: "456 lbs" }, tagline: "Championship pedigree" },
  { id: "spark-trixx-2024", category: "watercraft", make: "Sea-Doo", model: "Spark Trixx", year: 2024, price: 7799, mileage: 0, status: "in-stock", color: "Vapor Blue", featured: false, image: "🌊", specs: { engine: "900 ACE", power: "90 hp", capacity: "2 riders" }, tagline: "Built to play" },
];
  ...
  return (
    <div className="min-h-screen bg-[#0a0e0d] text-zinc-100">
      <style>{`
        ...
        .grid-bg {
          background-image: linear-gradient(rgba(74,222,196,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(74,222,196,0.04) 1px, transparent 1px);
          background-size: 56px 56px;
        }
        .diag-stripe {
          background: repeating-linear-gradient(45deg, rgba(74,222,196,0.08) 0 12px, transparent 12px 24px);
        }
      ...
      <div className="font-sans">
        ...
        <header className="border-b border-zinc-800 sticky top-0 z-30 bg-[#0a0e0d]/90 backdrop-blur-md">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 rounded-md bg-emerald-400 flex items-center justify-center -rotate-6">
            ...
            <nav className="hidden md:flex items-center gap-7 text-sm">
              ...
              <button onClick={() => setLeadOpen(true)} className="px-4 py-2 bg-emerald-400 text-black font-semibold rounded-md hover:bg-emerald-300 transition">
                Get Pre-Approved
              </button>
            </nav>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-6 py-20 md:py-32 relative">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10 mt-20 pt-10 border-t border-zinc-900">
              {[
                ...
                <div key={i}>
                  <div className="font-display text-5xl text-emerald-400 leading-none">{s.num}</div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {featured.map(v => (
                <button
                  ...
                  className="group text-left bg-gradient-to-br from-zinc-900 to-[#0a0e0d] border border-zinc-800 hover:border-emerald-400/50 rounded-xl p-6 transition-all hover:-translate-y-1"
                >
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <div className="font-mono text-[10px] uppercase tracking-widest text-emerald-400 mb-1">{v.year} · {v.category.toUpperCase()}</div>
                    ...
                    <div className="text-6xl opacity-50 group-hover:opacity-100 group-hover:scale-110 transition">{v.image}</div>
                  </div>
        ...
        <section id="inventory" className="max-w-7xl mx-auto px-6 py-20">
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-8">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full bg-zinc-950 border border-zinc-800 focus:border-emerald-400 rounded-md pl-11 pr-4 py-3 text-sm focus:outline-none transition"
              />
            </div>
            <div className="flex gap-2 overflow-x-auto">
              {CMS_CATEGORIES.map(c => (
                <button
                  ...
                  className={`px-4 py-3 text-xs font-mono uppercase tracking-wider rounded-md whitespace-nowrap transition ${
                    activeCategory === c.slug
                      ? "bg-emerald-400 text-black"
                      : "bg-zinc-950 border border-zinc-800 text-zinc-400 hover:border-emerald-400/50"
                  }`}
                >
                  {c.label}
                </button>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((v, i) => (
              <button
                key={v.id}
                onClick={() => setSelected(v)}
                className="group text-left bg-zinc-950/60 border border-zinc-800 hover:border-emerald-400 rounded-lg overflow-hidden transition-all hover:-translate-y-1 slide-in"
                style={{ animationDelay: `${i * 50}ms` }}
              >
                <div className="aspect-[16/10] bg-gradient-to-br from-zinc-900 to-[#0a0e0d] flex items-center justify-center text-7xl relative">
                  {v.image}
                  {v.status === "incoming" && (
                    <div className="absolute top-3 right-3 px-2 py-1 bg-amber-400/20 border border-amber-400/40 text-amber-400 text-[10px] font-mono uppercase tracking-wider rounded">
                      Incoming
                    </div>
                  )}
                  {v.status === "in-stock" && (
                    <div className="absolute top-3 right-3 px-2 py-1 bg-emerald-400/20 border border-emerald-400/40 text-emerald-400 text-[10px] font-mono uppercase tracking-wider rounded">
                      In Stock
                    </div>
                  )}
                </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20 text-zinc-500 font-mono text-sm">
              No matching units. Try adjusting your filters.
            </div>
          )}
        </section>
          ...
          <div className="max-w-7xl mx-auto px-6 py-20">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <button onClick={() => setLeadOpen(true)} className="inline-flex items-center gap-2 bg-emerald-400 text-black px-6 py-3 font-display text-xl rounded-md hover:bg-emerald-300 transition">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex gap-4 p-5 border border-zinc-800 rounded-lg hover:border-emerald-400/40 transition">
        ...
        <section id="about" className="border-t border-zinc-900">
          <div className="max-w-7xl mx-auto px-6 py-20">
            <div className="bg-emerald-400 text-black rounded-2xl p-10 md:p-16 relative overflow-hidden">
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-[#0a0e0d] border border-zinc-800 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="aspect-[16/9] bg-gradient-to-br from-zinc-900 to-[#0a0e0d] flex items-center justify-center text-9xl relative">
              {selected.image}
            ...
            <div className="p-8">
              <div className="font-mono text-[10px] uppercase tracking-widest text-emerald-400 mb-1">{selected.year} · {selected.category} · {selected.color}</div>
              ...
              <div className="flex items-end justify-between mb-6 pt-4 border-t border-zinc-900">
                <div>
                  ...
                  <div className="font-display text-5xl text-emerald-400">${selected.price.toLocaleString()}</div>
                </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => { setSelected(null); setLeadOpen(true); }} className="flex-1 bg-emerald-400 hover:bg-emerald-300 text-black px-6 py-3 font-display text-xl rounded-md transition">
                  APPLY FOR FINANCING
                </button>
      ...
      {leadOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setLeadOpen(false)}>
          <div className="bg-[#0a0e0d] border border-emerald-400/40 rounded-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
            <div className="p-8">
              ...
              <div className="space-y-3">
                ...
                <button onClick={() => setLeadOpen(false)} className="w-full bg-emerald-400 hover:bg-emerald-300 text-black py-3.5 font-display text-xl rounded-md transition mt-2">
